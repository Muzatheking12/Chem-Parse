...
class MolEditWidget(MolWidget):
    ...
    def event_handler(self, object1, object2):
        ...
        match (object1, object2, self.chemEntityType, self.action):
            ...
            case _:
                self.logger.warning(
                    f"Incorrect Mode For Execution. Undefined Action"
                )
    ...
    def add_atom_to_atom(self, atom, chemEntity=None):
        ...
        try:
            ...
            msg = str(e)
            if "Explicit valence for atom" in msg:
                StyledMessageDialog("Valence Error", msg, self).exec()
            else:
                StyledMessageDialog("Error", msg, self).exec()
            return None
